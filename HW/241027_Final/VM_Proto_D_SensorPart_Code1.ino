...
void setup() {
  ...
  BLE.setLocalName("PET-I Sensor"); // BLE 장치 이름 설정
...
void loop() {
  ...
  // BLE 연결 상태에 따른 LED 제어
  if (central) {
    // BLE 연결됨 - 하늘색 (GREEN + BLUE)
    ...
    snprintf(ptArrCompleteData[0], sizeof(ptArrCompleteData[0]), " %.2f| %.1f V|", objectTemp, beatsPerMinute); // 0번째 요소: objectTemp
    snprintf(ptArrCompleteData[1], sizeof(ptArrCompleteData[1]), " %.2f| %.2f| %.2f A", ax, ay, az); // 2번째 요소: ax, ay, az
    snprintf(ptArrCompleteData[2], sizeof(ptArrCompleteData[2]), "%.2f| %.2f| %.2f!", gx, gy, gz); // 3번째 요소: gx, gy, gz
